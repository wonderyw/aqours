...
bool Reader::isEOF()  ///< @wonderyw ÊÇ·ñ½áÊø
{
	return (m_pos >= m_len);
}
...
char Reader::readChar()  ///< @wonderyw ¶ÁÈ¡Ò»¸ö×Ö·û
{
	if (isEOF())
...
char Reader::peekChar()  ///< @wonderyw Ô¤¶ÁÈ¡Ò»¸ö×Ö·û
{
	if (isEOF())
		throw ReaderException("Reader::readChar", "EOF.", getLine(), getRow());
	char tRet = m_str[m_pos];
...
void Reader::ignoreSpace()  ///< @wonderyw Ìø¹ý¿Õ°××Ö·û
{
	while (!isEOF())
	{
		char tRet = peekChar();
